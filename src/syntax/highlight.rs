...
pub fn get_highlight_config(
    lang: LapceLanguage, grammars_directory: &Path, queries_directory: &Path
) -> Result<Arc<HighlightConfiguration>, HighlightIssue> {
    HIGHLIGHT_CONFIGS.with(|configs| {
        let mut configs = configs.borrow_mut();
        let config = configs
            .entry(lang)
            .or_insert_with(|| new_highlight_config(
                get_grammar(&lang.grammar_name(), &lang.grammar_fn_name(), lang.name(), grammars_directory),
                &lang.query_name(), crate::language::LapceLanguage::HIGHLIGHTS_INJECTIONS_FILE_NAME, LapceLanguage::HIGHLIGHTS_QUERIES_FILE_NAME, lang.name(),queries_directory
            ).map(Arc::new));
        config.clone()
    })
}
...
pub fn new_highlight_config(
    ...
    query_name: &str, highlights_injections_file_name: &str, highlights_queries_file_name: &str, language_name: &str, queries_directory: &Path
) -> Result<HighlightConfiguration, HighlightIssue> {
    let grammar = grammar.ok_or(HighlightIssue::NotAvailable)?;
    let (query, injection) = get_grammar_query(query_name, highlights_injections_file_name, highlights_queries_file_name, queries_directory);

    match HighlightConfiguration::new(grammar, &query, &injection, "") {
        Ok(x) => Ok(x),
        Err(x) => {
            let str = format!("Encountered {x:?} while trying to construct HighlightConfiguration for {}", language_name);
        ...
        for capture in query_match.captures {
            ...
            if index == self.injection_language_capture_index {
                ...
                static SHEBANG_REGEX: LazyLock<Regex> =
                    LazyLock::new(|| Regex::new(SHEBANG).unwrap());
